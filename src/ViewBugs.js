...
function ViewBugs() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`https://bugtracker-d0jq.onrender.com/bugs/${editingBug.id}`, updatedBug);
  ...
  useEffect(() => {
    const fetchBugs = async () => {
      try {
        const response = await axios.get("https://bugtracker-d0jq.onrender.com/bugs");
